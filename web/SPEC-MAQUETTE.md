...
> **RÉVISION 2026-08-09 — retour vocal de l'association (2026-08-08).** Vocabulaire : « structures qui nous accueillent » → **« structures partenaires »** partout (registre collaboratif) ; « pas de gymnase attitré » → **« en attendant nos locaux »**. **Cinquième action ajoutée : `projets`** (slug/ancre) — projets inédits co-construits avec une structure partenaire à partir de son besoin ; l'exemple cité est l'exposition montée avec une structure. Les temps d'échange sont **thématisés** : la thématique est définie en amont avec la structure à partir d'une problématique identifiée (jamais « discuter pour discuter »). `/interventions` : « initiation ou autre action **en structure encadrante** (foyer, centre social, école, insertion…) » ; « ou définir un projet ensemble » ajouté aux formats de chaque cible. Équipe élargie (`lib/data/coachs.ts` + champ `metier`) : Younes → **Conseiller en développement**, ajout de **Douka** (psychologue) et **Hugo** — infos à confirmer, voir `A-VALIDER.md`. Quiz accueil : « votre programme » → **« l'action qu'il vous faut »**. `/adhesion` : fiche d'inscription PDF téléchargeable en complément du formulaire, cotisation précisée (participants d'une structure partenaire pris en charge par la structure).

> **RÉVISION 2026-07-06 (bis) — purge du contenu inventé.** Après retour de l'association (brochure `docs/Dossier Soungui_merged (1) (1).pdf` + consignes) : **ne rien inventer — si une info n'est pas connue, on ne met rien.** Chiffres réels : 30+ actions en structures, 5 structures partenaires, 2 ans d'existence. Équipe réelle : Soungui Gomis (président fondateur), Younes (boxeur amateur & coach), Zinedine Meftah (boxeur amateur, passif de coach) — sans diplômes ni bios inventés (le §3 « Coachs maquette (4) » ci-dessous est OBSOLÈTE, comme les compteurs 450/25/12/300 et les stages nommés). Pas de gymnase attitré. Actions réalignées sur la brochure : initiations & cours (avec temps d'échange/discussion — à mettre en avant), stages avec les structures partenaires, temps d'échanges & sorties, galas amicaux (ambition). Les actions en foyer sont la première cible de `/interventions`. Le vrai logo (`public/images/logo-cbac.jpg`) est utilisé dans le header. Témoignages, partenaires nommés, actus et événements datés : supprimés tant que l'association ne les fournit pas (états « en préparation »). Voir `A-VALIDER.md`.
